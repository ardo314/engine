        ...
        for item in items {
            match item {
                TopLevelItem::TypeAlias(ta) => {
                    self.type_aliases.entry(ta.name.clone()).or_insert_with(|| ta.clone());
                }
                TopLevelItem::Enum(e) => {
                    self.enums.entry(e.name.clone()).or_insert_with(|| e.clone());
                }
                TopLevelItem::Variant(v) => {
                    self.variants.entry(v.name.clone()).or_insert_with(|| v.clone());
                }
                TopLevelItem::Flags(f) => {
                    self.flags.entry(f.name.clone()).or_insert_with(|| f.clone());
                }
                TopLevelItem::Record(r) => {
                    ...
                    self.records.entry(r.name.clone()).or_insert_with(|| r.clone());
                }
                TopLevelItem::System(s) => {
                    self.systems.entry(s.name.clone()).or_insert_with(|| s.clone());
                }
